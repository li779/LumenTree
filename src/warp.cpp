...
static float inline sign (const Point2f& p1, const Point2f& p2, const Point2f& p3)
{
...
float Warp::squareToUniformDiskPdf(const Point2f &p) {
    return (p.x() * p.x() + p.y() * p.y() <= 1) ? INV_PI : 0.f;
}
...
float Warp::squareToUniformSpherePdf(const Vector3f &v) {
    return (std::abs((v.x() * v.x() + v.y() * v.y() + v.z() * v.z()) - 1.f) < Epsilon) ? INV_FOURPI : 0.f;
}
